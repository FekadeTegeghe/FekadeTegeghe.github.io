...
const arrP = require("./arrayPractice.js");
...
describe("map practice", function () {
    ...
    it("doubles an array of numbers", function () {
        assert.deepStrictEqual(arrP.doubleNums(numArray), [10, 0, 14, 154, -40, 600, 102, 4]);
        assert.deepStrictEqual(numArray, [5, 0, 7, 77, -20, 300, 51, 2]);  //test for pure function
    });

    it("doubles age", function () {
        assert.deepStrictEqual(arrP.doubleAges(peopleArray),
            [{ name: "Sam", age: 30 }, { name: "William", age: 12 }, { name: "Lucy", age: 26 }, { name: "Barney", age: 160 }]);
        assert.deepStrictEqual(peopleArray,
...
describe("filter practice", function () {
    ...
    it("filter all even numbers", function () {
        assert.deepStrictEqual(arrP.filterEven(numArray), [0, -20, 300, 2]);
        assert.deepStrictEqual(numArray, [5, 0, 7, 77, -20, 300, 51, 2]);  //test for pure function
    });

    it("filter all age > 10", function () {
        assert.deepStrictEqual(arrP.filterOver10(peopleArray),
    ...
    it("find even", function () {
        assert.strictEqual(arrP.findEvenNum(numArray), 0);
        assert.strictEqual(arrP.findEvenNum([1, 3, 801]), undefined);
    });
    it("find even age ", function () {
        assert.deepStrictEqual(arrP.findEvenAge(peopleArray), { name: "William", age: 6 });
        const peopleOddAge = [{ name: "Sam", age: 15 }, { name: "Lucy", age: 13 }, { name: "Barney", age: 81  }];
        assert.deepStrictEqual(arrP.findEvenAge(peopleOddAge), undefined);
    });
